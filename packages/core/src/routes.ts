...
export class Router<Ext = unknown> {
  ...
  private readonly children: Router<Ext>[] = []
  ...
  route(
    method: HttpMethod,
    path: string,
    b: RouteSchema | Handler<Ext>,
    c?: ValidatedHandler<RouteSchema, Ext>,
  ): this {
  ...
  use(child: Router<Ext>): this
  use(value: Middleware | Router<Ext>): this {
  ...
  collect(basePrefix = '', inherited: RouterOptions = {}): CollectedRoutes {
    ...
    const routes: CollectedRoute[] = this.entries.map((entry) => ({
      ...
      schema: merge(entry.schema, tags, auth),
    ...
    for (const child of this.children) {
      const collected = child.collect(prefix, { tags, auth })
...
function merge(
  ...
  if (!hasTags && !hasAuth) return schema
  if (!schema) {
    return {
      ...(hasTags ? { tags: [...tags] } : {}),
      ...(hasAuth ? { auth } : {}),
