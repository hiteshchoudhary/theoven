...
Gives you `ctx.storage` — upload, download, list, delete, stat, and presigned URLs — over four
drivers: S3 (and anything S3-compatible), local disk, [Bunny](/docs/bricks/storage-bunny/) and
[ImageKit](/docs/bricks/storage-imagekit/). One interface, so the driver is a config change.
...
This brick adds no endpoints. Files are served from your storage provider or through your own
routes; nothing is mounted automatically, because a storage brick that silently exposed a public
file route would be a security decision made on your behalf.

## What it creates

**Files:** with `diskStorage`, the directory at `dir` and everything uploaded into it — add it to
`.gitignore`. With the other drivers, nothing locally.

**Tables:** none. This brick stores no index of what you uploaded; the bucket is the source of
truth.
...
- **No copy or move.** Read and write, or reach through [`ctx.storage.raw`](/docs/reference/bricks/#raw--the-escape-hatch) — which ties that code to one driver.
