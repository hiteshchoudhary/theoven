...
Google and GitHub are optional flows on this brick. Two extra steps beyond the configuration on the
[`auth` page](/docs/bricks/auth/#social-sign-in):

**1. Add the accounts table to your schema.** It lives on a separate export path, so an application
that never uses social sign-in never gets it:
...
**2. Configure a provider.** That is the opt-in — it turns on the store's account methods, and the
brick then checks at boot that you did step 1 too.

<Aside type="note" title="Two steps, on purpose">
  Configuring a provider without the schema fails at **startup**, naming both fixes. The
  alternative — shipping the table to everyone — means a migration for applications that will never
  have a linked account, and the line being in *your* schema file is what keeps it from appearing
  in `git status` unexplained.
</Aside>
...
[`user.raw`](/docs/bricks/auth/#userraw--the-escape-hatch) is the stored row, so any column you added to `auth_users` is reachable — and is what a
