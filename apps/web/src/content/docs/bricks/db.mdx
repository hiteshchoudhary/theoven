...
A [dependency](/docs/reference/dependencies/) scopes it to exactly the routes that name it,
...
`healthPath` mounts an endpoint. `checkHealth` is the same check, for when you want it inside one
of your own:
...
It issues a **real query** — `select 1`, or a `ping` on Mongo — rather than asking whether a pool
