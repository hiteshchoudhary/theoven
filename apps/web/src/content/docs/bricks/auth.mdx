...
The eight endpoints are the common case. Everything behind them is exported, for the cases where
you need your own endpoint but not your own security.
...
Each takes `ctx.auth.flows` — the same `FlowConfig` the mounted endpoints use, so password rules,
...
```ts title="src/routes/settings/unlink.post.ts"
...
It refuses with a `409` when that provider is the person's **last** way of signing in — removing it
would lock them out of their own account, and no confirmation dialog makes that recoverable.

<Aside type="note" title="There is no matching link endpoint yet">
  Attaching a provider to the account you are already signed in as is not built. Linking happens
  through signing in, so a password user has to sign out first. Tracked, and the shape above is
  where it will go.
</Aside>
...
argon2id with Bun's default cost parameters — not numbers we invented. `verifyPassword` returns
