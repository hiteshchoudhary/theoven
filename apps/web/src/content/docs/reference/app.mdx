...
`.use()` takes all three, and tells them apart with no ambiguity: a function is middleware, a
...
that line and is a compile error before it. A [router](/docs/reference/routers/) contributes
...
Replaces a [dependency](/docs/reference/dependencies/) for this app — the testing seam:
...
It applies wherever the dependency is used, including as a sub-dependency of another, so replacing
one root swaps out everything derived from it without touching those definitions.
...
introduced it. See [response schemas](/docs/reference/validation/#response-validation).
