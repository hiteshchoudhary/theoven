...
An `onError` handler receives whatever was thrown, which is not necessarily an `OvenError` — a
...
Anything that is already an `OvenError` is returned unchanged. Anything else becomes a `500` whose
