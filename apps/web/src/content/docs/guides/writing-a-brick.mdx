...
Runs **after routing** — so `ctx.params` is available — and **before** middleware, validation
and the handler. Throwing rejects the request, which is how an auth brick turns a bad token into
a `401` without a handler ever seeing it.
...
```ts
export function cache(): Brick<'cache', CacheClient> {
  return {
    name: 'cache',
    dependsOn: ['db'],
    setup: (context) => createCache(context.resolved.db as Database),
  }
}
```
